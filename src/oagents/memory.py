...
class PlanningStep(MemoryStep):
    ...
    def to_messages(self, summary_mode: bool, **kwargs) -> List[Message]:
        ...
        messages.append(
            # Message(
            #     role=MessageRole.ASSISTANT, content=[{"type": "text", "text": f"[FACTS REASONING AND THINKING PROCESS]:\n{self.facts_think.strip()}\n\n[FACTS RESULT LIST]:\n{self.facts.strip()}"}]
            # )
            Message(
                role=MessageRole.ASSISTANT, content=[{"type": "text", "text": f"[FACTS RESULT LIST]:\n{self.facts.strip()}"}]
            )
        )
        ...
        if not summary_mode:  # This step is not shown to a model writing a plan to avoid influencing the new plan
            # messages.append(
            #     Message(
            #         role=MessageRole.ASSISTANT, content=[{"type": "text", "text": f"[PLAN REASONING AND THINKING PROCESS]:\n{self.plan_think.strip()}\n\n[PLAN RESULT]:\n{self.plan.strip()}"}]
            #     )
            # )
            messages.append(
                Message(
                    role=MessageRole.ASSISTANT, content=[{"type": "text", "text": f"[PLAN RESULT]:\n{self.plan.strip()}"}]
                )
            )
        return messages
...
class ReflectionStep(MemoryStep):
    model_input_messages: List[Message]
    history_trajectory_analysis: str
    history_trajectory_score: int
    facts: str
    facts_think: str
    plan: str
    plan_think: str
    ...
    def to_messages(self, summary_mode: bool, **kwargs) -> List[Message]:
        ...
        messages.append(
            Message(
                role=MessageRole.ASSISTANT, content=[{"type": "text", "text": f"[HISTORY TRAJECTORY ANALYSIS]:\n{self.history_trajectory_analysis.strip()} \n [HISTORY TRAJECTORY SCORE]:\n{self.history_trajectory_score}"}]
            )
        )
